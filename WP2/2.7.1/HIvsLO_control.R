...
# This takes as input the genotype matrix preprocessed with DiGAS and metabolomics data.
# In this script netDx is trained on 90% of CN samples -> 307 CN samples (half HI, half LO). 
# The separation between high and low pollution was performed in the preprocessing/prepare_pollution_groups.R script.
...
# Parameters
nco<-5
run_name<-'CN_POLLUTANTS_HILO'

# 1) PREPARE DATA 
## list with assays and phenotype. Should be named with assay names and  "pheno"
pheno<-read.table("/home/cmengoni/pollutants/CN_all_dataset_BALANCED.tsv",header = 1)
pheno$ID <- paste0('ID_',pheno$ID)
rownames(pheno) <- pheno$ID

## read genotype
geno<-readRDS("/home/cmengoni/pollutants/genotype_matrix.rds")
pheno <- pheno[pheno$ID %in% colnames(geno),]
geno<-geno[,rownames(pheno)] #put them in the same order as meta and pheno
#head(geno)

## read meta
meta<-read.table("/home/cmengoni/INC_BOTH_AD_CL/metabolomics_preprocessed.txt",header = 1) # can also try to group by metablites ubgroup
meta<-meta[,rownames(pheno)] #put them in the same order as meta and pheno
...
test_hi<-sample(pheno[pheno$STATUS=='HI',"ID"],0.2*length(pheno[pheno$STATUS=='HI',"ID"]))
test_lo<-sample(pheno[pheno$STATUS=='LO',"ID"],0.2*length(pheno[pheno$STATUS=='LO',"ID"]))
test<-c(test_hi,test_lo)
#training_IDs<-pheno_original[!(pheno_original$ID %in% test),]$ID
dsets<-create_holdout_from_pheno(dataList,test)#training_IDs)
